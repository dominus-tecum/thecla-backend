...
def get_image_uuid_from_bytes(image_data):
    """Extract UUID from image bytes"""
    try:
        tags = exifread.process_file(BytesIO(image_data), details=False)
        uuid = str(tags.get('EXIF ImageUniqueID', tags.get('Image ImageUniqueID', '')))
        return uuid if uuid and uuid != 'None' else None
    except:
        return None

def create_uuid_mapping(docx_path):
    """Extract images from docx and create UUID mapping"""
    mapping = {}
    print("\n🔧 Creating UUID mapping...")
    
    with zipfile.ZipFile(docx_path, 'r') as docx_zip:
        for file_info in docx_zip.filelist:
            if file_info.filename.startswith('word/media/') and any(ext in file_info.filename for ext in ['.png', '.jpg', '.jpeg']):
                ...
                if uuid and uuid != 'None':
                    ...
                    print(f"   UUID: {uuid}")
                    marker = input("   Enter marker (e.g., P9.1): ").strip()
                    mapping[marker] = uuid
    
    with open('image_uuid_map.json', 'w') as f:
        json.dump(mapping, f, indent=2)
    print("\n✅ UUID mapping saved to image_uuid_map.json")
    return mapping
            ...
            for file_info in docx_zip.filelist:
                ...
                if filename.startswith('word/media/') and any(ext in filename.lower() for ext in ['.png', '.jpg', '.jpeg', '.gif', '.bmp']):
                    try:
                        ...
                        # KEEP ORIGINAL FILENAME - DON'T RENAME
                        original_filename = os.path.basename(filename)
                        
                        # For upload, use a clean name but keep original for reference
                        image_filename = original_filename
                        image_base64 = base64.b64encode(image_data).decode('utf-8')
                        
                        images.append({
                            'original_filename': original_filename,  # This is the truth
...
def enhance_content_with_media(text_content, images, tables, exam_title):
    print(f"\n🔍 DEBUG: Received {len(tables)} tables")
    enhanced_content = text_content
    
    if images:
        # Sort images by image number
        def get_image_number(img):
            match = re.search(r'image(\d+)', img['filename'])
            return int(match.group(1)) if match else 999
        
        images.sort(key=get_image_number)
        
        print(f"\n🔍 IMAGES SORTED BY NUMBER:")
        for idx, img in enumerate(images, 1):
            size_kb = len(img['data']) * 3 / 4 / 1024
            print(f"   {idx}. {img['filename']} - {size_kb:.0f} KB")
        
        # Find markers
        markers = re.findall(r'\[(\d+)\]', enhanced_content)
        print(f"\n🔍 MARKERS FOUND: {[f'[{m}]' for m in markers]}")
        ...
        for idx, marker_num in enumerate(markers):
            if idx < len(images):
                ...
                enhanced_content = enhanced_content.replace(f'[{marker_num}]', img_markdown)
                print(f"   ✓ Replaced [{marker_num}] with {img['filename']} ({size_kb:.0f} KB)")
    ...
    if tables:
        for i, table in enumerate(tables, 1):
            ...
            if table_markdown:
                ...
                print(f"   📊 Added table {i} to content")
    # ========== END ADD ==========
...
def upload_single_file(file_path, filename, discipline_id, discipline_name, base_url):
    ...
    try:
        ...
        if filename.lower().endswith('.docx'):
            ...
            # ========== EXACT DEBUG - COPY THIS ==========
            print("\n" + "="*60)
            print("DEBUG: CHECKING FOR 9.8.1 AND 9.9")
            print("="*60)
            
            # Check if 9.8.1 exists
            if '9.8.1' in full_text:
                print("✅ '9.8.1' FOUND in text")
                pos = full_text.find('9.8.1')
                print(f"   Position: {pos}")
                print(f"   Text from position 0 to {pos+500}:")
                print(full_text[max(0, pos-200):pos+500])
            else:
                print("❌ '9.8.1' NOT FOUND in text")
            
            # Check if 9.9 exists
            if '9.9' in full_text:
                print("\n✅ '9.9' FOUND in text")
                pos2 = full_text.find('9.9')
                print(f"   Position: {pos2}")
                print(f"   Text around 9.9:")
                print(full_text[max(0, pos2-200):pos2+200])
            else:
            ...
            # ========== ADD THIS RIGHT HERE ==========
            # Check if UUID mapping exists, if not, create it
            if not os.path.exists('image_uuid_map.json'):
                create_uuid_mapping(file_path)
            # ========== END ADD ==========

            # Sort images by numeric order (image1, image2, image3...)
                        # Sort images by numeric order (image1, image2, image3...)
            ...
            print("\n🔍 IMAGE SIZES (raw):")
            for idx, img in enumerate(images, 1):
                raw_size_kb = len(img['data']) * 3 / 4 / 1024
                print(f"  Image {idx}: {img['filename']} - {raw_size_kb:.0f} KB")
            # ========== END SIZE DEBUG ==========
            ...
            for img in images:
                if img['mimetype'] == 'image/png':
                    try:
                        ...
                        # Update image data
                        img['data'] = base64.b64encode(compressed_data).decode('utf-8')
                        img['mimetype'] = 'image/jpeg'
                        img['filename'] = img['filename'].replace('.png', '.jpg')
                        compressed_count += 1
                        print(f"      Compressed {img['filename']} to JPEG")
                    except Exception as e:
                        print(f"      Could not compress {img['filename']}: {e}")
            print(f"   Compressed {compressed_count} PNG images to JPEG")
            # ========== END COMPRESSION ==========


            print("Sorted images order:")
            for idx, img in enumerate(images):
                print(f"  {idx+1}: {img['filename']}")
        ...
        for i, section in enumerate(sections):
            print(f"   Section {i}: {section.get('text', 'NO TITLE')[:50]}")




        print(f"\n🔍 CHECKING SECTIONS FOR TABLES:")
        for i, section in enumerate(sections):
            content = section.get('options', [''])[0] if section.get('options') else ''
            if '**Table' in content or '|' in content:
                print(f"   Section {i}: CONTAINS TABLE")
            else:
                print(f"   Section {i}: NO TABLE")

        print(f"\n🔍 LAST 500 CHARACTERS OF ENHANCED_TEXT:")
        ...
        if images:
            ...
            for i, img in enumerate(images, 1):  # ← REMOVE [:3]
        ...
        print(f"\n📤 Uploading to server...")
        response = requests.post(f"{base_url}/exam/", json=payload, timeout=120)
